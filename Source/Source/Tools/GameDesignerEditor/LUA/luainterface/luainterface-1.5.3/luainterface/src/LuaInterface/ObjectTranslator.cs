namespace LuaInterface 
{
	...
	public class ObjectTranslator 
	{
        ...
        private Hashtable m_mapComAssembly = new Hashtable();
        ...
        //尝试以com组件的方式进行载入
        private bool RegComDll(string fileName)
        {
            Process.Start("regsvr32.exe", "-s " + fileName);
            return true;
        }
        ...
        #region ".NET的组件"
        private bool TryLoadAssemByFileName(string dllname)
        {
            try
            {
                string strDir = Path.GetDirectoryName(Process.GetCurrentProcess().MainModule.FileName);

                //add by cuiwei 07.8.20
                string sd = findFullPath(strDir, dllname);
                strDir = sd != "" ? sd : strDir;
                //add end

                Assembly assembly = Assembly.LoadFile(strDir + "\\" + dllname);
                
                if (assembly != null)
                {
                    //assemblies.RemoveAt(assemblies.BinarySearch(assembly)); // 为什么这句不行?
                    for (int i = 0; i < assemblies.Count; i++)
                    {
                        if (assemblies[i] == assembly)
                        {
                            assemblies.RemoveAt(i); //修改后无须关闭虚拟机也可立即使用新版本
                            break;
                        }
                    }
        ...
        //add by cuiwei 07.8.20
        //搜索所有子目录中指定的DLL文件名，遇到第一个相符的DLL则立即返回
        private string findFullPath(string strPath, string dllName)
        {
            if(File.Exists(strPath + "\\" + dllName))
            {
                return (strPath);
            }
            ...
            {
                string sf = findFullPath(d.FullName, dllName);
                if (sf != "") 
                {
                    return (sf);
                }
			...
			try 
			{
                ...
                if (!TryLoadAssemByFileName(assemblyName))
                {
                    ...
                    try
                    {
                        ...
                        if (assembly == null)
                        {
                            assembly = loadComAssembly(assemblyName);
                        }
                    }
                    catch (Exception)
                    {
        ...
        internal Type FindType(string className)
        {
			...
			{
                //com组件的interop.dll会自动生成class后缀，需要优先考虑
                Type klass = null;
                if (m_mapComAssembly.Contains(assembly.FullName)) //是com组件，则优先加Class查找
                {
                    try
                    {
                        klass = assembly.GetType(className + "Class", true, true);
                    }
                    catch{}
                }
